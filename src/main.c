...
int main(const int argc, char **argv)
{
    ...
    log_message(main_logger, INFO, SETUPCONF, "Configuring Server Address");
    ...
    if (inet_pton(AF_INET, g_conf->conf_addr, &(conf_addr.sin_addr)) <= 0)
    {
        log_message(main_logger, ERROR, SETUPCONF, "Invalid configuration server address format: %s", g_conf->conf_addr);
        return 1;
    }
    ...
    log_message(main_logger, INFO, SETUPCONF, "Creating UDP Socket");
    ...
    log_message(main_logger, INFO, SETUPCONF, "Binding Socket to the specified address and port");
